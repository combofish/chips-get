...
int j;
unsigned char serial_slave_temp[10] = {0};
unsigned char serial_slave_send[10] = {0};
  ...
  if(RI) //再次判断，是否接收到数据(接收到数据后，RI会置1，需手动清0)
      ...
      if(RXstart) //判断是否接收到过本地址
	{
	  if(RXData != '$') //判断是否接收到 数据结束 标志 $
	    {
	      serial_slave_temp[j] = RXData; //没有接收到结束标志，正常保存数据至数组
	      j++;
	    }
	  else //接收到 结束标志 $
	    {
	      ...
	      // end receive.
	      //TXData("04567$");
		  TXData(serial_slave_send);
	    }
	}

      if(RXData == Slave_addr) //判断是否呼叫本机，地址范围：000 – 254(00 - FE)
	{
	  RXstart = 1; //开始接收数据
	  SM2 = 0; //配置为：接收数据 模式
	}
